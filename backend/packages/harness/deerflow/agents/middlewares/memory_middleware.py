...
本模块实现了 Agent 执行完成后的异步记忆更新机制。核心思路是：
在每次 Agent 对话回合结束后，将经过过滤的对话消息入队，
由后台的防抖队列（debounce queue）批量处理，通过 LLM 总结提取用户偏好、
事实和上下文信息，并原子性地写入持久化存储。

关键设计要点：
- 仅保留用户输入和最终助手回复，过滤掉工具调用等中间过程消息，
  以避免噪声信息干扰记忆提取质量。
- 在入队时捕获 user_id，而非在后台 Timer 线程中读取 ContextVar，
  因为 Python 的 threading.Timer 在新线程上执行，ContextVar 的值
  不会自动传播到新线程上下文中。
- 支持按 Agent 维度隔离记忆（通过 agent_name 参数），也可使用全局记忆。
"""
